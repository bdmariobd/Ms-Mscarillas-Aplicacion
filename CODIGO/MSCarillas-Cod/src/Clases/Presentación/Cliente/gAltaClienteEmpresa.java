 ...
 */
public class gAltaClienteEmpresa extends JFrame {
	...
	public void init() {
		...
		JLabel lblNewLabel = new JLabel("Alta de cliente empresarial");
		...
		panel_1.setLayout(new GridLayout(6, 4, 5, 30));
		...
		btnAceptar.addActionListener(new ActionListener(){
			public void actionPerformed(ActionEvent e) {
				try {
					...
					if(formattedTextField_1.getText().length() != 9) { 
						throw new Exception("El CIF debe ser 9 dígitos");
					}
					cif = (String) formattedTextField_1.getText();
					...
					if(formattedTextField_5.getText().length() >= 51 ) { 
					...
					Controller.getInstance().action(
						new Contexto(
								Events.ALTA_CLIENTEE, new TClienteEmpresa(telf, true, correo, direccion, ncuenta, nombre, cif)));
				}
				catch(Exception es) {
