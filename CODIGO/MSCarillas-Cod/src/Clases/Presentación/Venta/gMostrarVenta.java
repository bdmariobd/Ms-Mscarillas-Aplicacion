...
public class gMostrarVenta extends JFrame {
	private static final long serialVersionUID = 1L;
	private JPanel contentPanel;
	
	public gMostrarVenta() {
		//update();
	}
	...
	public void init(TVenta actual) {
		...
		if(actual!=null) lblNewLabel_1 = new JLabel("Identificador. Escriba 0 para mostrar la venta actual");
		else lblNewLabel_1 = new JLabel("Identificador");
		lblNewLabel_1.setFont(new Font("Consolas", Font.PLAIN, 20));
		lblNewLabel_1.setHorizontalAlignment(SwingConstants.CENTER);
		panel_1.add(lblNewLabel_1);
		...
		btnAceptar.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				try {
					...
					if(actual==null&&id==0) throw new Exception("No hay venta actual");
					Controller.getInstance().action(new Contexto(Events.MOSTRAR_VENTA,id));
				}
				catch(Exception ex) {
