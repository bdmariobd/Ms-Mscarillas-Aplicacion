...
public class gBorrarProductoVenta extends JFrame{
	private static final long serialVersionUID = 1L;
	private JPanel contentPanel;
	private TVenta ventaActual;
	public gBorrarProductoVenta() {
		//init();
	}
	public void init(TVenta ventaActual) {
		...
		JLabel lblNewLabel = new JLabel("Borrar producto de la venta actual");
		...
		btnAceptar.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				try {
					...
					if (!ventaActual.getCarrito().containsKey(idProducto)) {
						throw new Exception("Este producto no esta en esta venta");
					}
					...
					int cActual = ventaActual.getCarrito().get(idProducto).getCantidad();
					if(cActual<=cantidad) {
						ventaActual.getCarrito().remove(idProducto);
						JOptionPane.showMessageDialog(null, "Como la cantidad es mayor o igual, se elimina el producto del carrito","EXITO",JOptionPane.WARNING_MESSAGE);
					}
					else {
						ventaActual.getCarrito().get(idProducto).setCantidad(cActual-cantidad);
						JOptionPane.showMessageDialog(null, "Se eliminaron "+cantidad+" unidades de "+cActual,"EXITO",JOptionPane.WARNING_MESSAGE);
					}
