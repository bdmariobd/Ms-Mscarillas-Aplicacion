...
public class gMostrarCursosEmpresa extends JFrame{
	private static final long serialVersionUID = 1L;
	private JPanel contentPanel;
	private JPanel panel;
	private Integer idEmp;
	public gMostrarCursosEmpresa() {
		//update();
	}
	public void init() {
		...
		JLabel lblNewLabel = new JLabel("Mostrar Cursos de la Empresa");
		...
		btnAceptar.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				try {
					idEmp = Integer.parseInt(formattedTextField.getText());
					Controller.getInstance().action(new Contexto(Events.MOSTRAR_CURSOS_EMPRESA, idEmp));
				}
				catch(Exception ex) {
	...
	public void update(Contexto contexto) {
		...
		JLabel lblNewLabel = new JLabel("Mostrar los cursos de la empresa "+idEmp);
		lblNewLabel.setHorizontalAlignment(SwingConstants.CENTER);
		lblNewLabel.setForeground(new Color(255, 255, 255));
		lblNewLabel.setFont(new Font("Consolas", Font.PLAIN, 32));
		contentPanel.add(lblNewLabel, BorderLayout.NORTH);
		...
		String[] cols = {"ID", "Nombre", "Tipo", "Descripcion", "Horas", "Nivel", "Activo"};
		...
		for (TCurso curso :listaCursos) {
			...
			Object[] objsCurso = {id, nombre, tipo, descripcion, horas, nivel, activo};
